...
class CityUtil:
    ...
    def cities_for_position(cls, game_state: CarcassonneGameState, city_position: CoordinateWithSide):
        tile = game_state.get_tile(city_position.coordinate.row, city_position.coordinate.column)
        cities = []
        if tile is None:
            return cities
        for city_group in tile.city:
            if city_position.side in city_group:
                ...
                for city_group_side in city_group:
                    p: CoordinateWithSide = CoordinateWithSide(city_position.coordinate, city_group_side)
                    cities.append(p)
        return cities
    ...
    def find_cities(cls, game_state: CarcassonneGameState, coordinate: Coordinate, sides: [Side] = (Side.TOP, Side.RIGHT, Side.BOTTOM, Side.LEFT)):
        # print('find cities', coordinate)
        cities: Set[City] = set()

        tile = game_state.get_tile(coordinate.row, coordinate.column)
